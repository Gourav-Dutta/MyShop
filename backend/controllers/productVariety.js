...
async function handleUpdateVarietyByVarietyIdOfThatSeller(req, res){
      try {
        ...
        const {color, size, product_id, weight, liter, price, sku, stock} = req.body;
        const updateData = {};
...
async function handleDeleteVarietyByVarietyIdOfThatSeller(req, res){
    ...
    if(!variety) return res.status(403).json({ "message" : "You are not authorized to perform this action"})
